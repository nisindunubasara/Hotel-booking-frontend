...
const NavBar = () => {
    ...
    return (
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                //always fixed top eke thiyenawa
                //full width
                //isScrolled true unama tika mulin, false unama tika ilagata

                {/* Logo */}
                <Link to='/'>//logo eka click karama home page ekata yanawa
                ...
                <div className="hidden md:flex items-center gap-4 lg:gap-8">
                    ...
                    { user && (
                        <button className={`border px-4 py-1 text-sm font-light rounded-full cursor-pointer ${isScrolled ? 'text-black' : 'text-white'} transition-all`} onClick={()=> isOwner ? navigate('/owner') :setShowHotelReg(true) }>
                           {isOwner ? "Dashboard" : "List Your Hotel"}//owner nm dashboard ekata yanawa, owner naththam hotel registration form eka pennanawa
                        </button>
                    )}
                </div>
